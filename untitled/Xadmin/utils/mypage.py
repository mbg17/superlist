class Pagination(object):
    """自定义分页（Bootstrap版）"""
    def __init__(self, page_num, all_data, base_url,param, per_page=10, max_page=11):
        """
    ...
    def page_html(self):
        ...
        if page_end >= self.all_page:
            # if self.all_page>self.max_page:
            #     page_start = self.all_page - self.max_page + 1
            # page_start=self.all_page
            page_end = self.all_page
        list_ = []
        # 拼接HTML
        list_.append(f'<li><a href="{self.base_url}?page=1">首页</a></li>')
        if self.page_num > 1:
        ...
        for i in range(page_start, page_end + 1):
            self.param['page']=i
            if i == self.page_num:
                list_.append(f'<li class="active"><a href="{self.base_url}?{self.param.urlencode()}">{i}</a></li>')
            else:
                list_.append(f'<li><a href="{self.base_url}?{self.param.urlencode()}">{i}</a></li>')
        ...
        list_.append(f'<li><a href="{self.base_url}?page={self.pages}">尾页</a></li>')
